...
class FarmaciaChart extends StatefulWidget {
  @override
  _FarmaciaChartState createState() => _FarmaciaChartState();
}

class _FarmaciaChartState extends State<FarmaciaChart> {
  ...
  Future loadJson() async {
    final jsonFile = await JsonPath().localFileFarmaciaJson;
  ...
  Widget _despChartList(BuildContext context, int index, String nameOrder) {
    ...
    if (lengthList != index || index == 0){
      nomeFarmacia = "Farmacia: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][datasTxt]["Valores"][0]} ";
    }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            if(lengthList != index || index == 0)(
                tabelaGastos(nomeFarmacia, result)
            )else(
                tabelaResult(result)
            )
          ],
  ...
  Widget tabelaGastos(String nomeFarmacia, result){
    return Container(
