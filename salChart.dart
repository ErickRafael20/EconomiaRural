...
class SalChart extends StatefulWidget {
  @override
  _SalChartState createState() => _SalChartState();
}

class _SalChartState extends State<SalChart> {
                      ...
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          ...
                          setState(() {
                            ...
                            fileLength = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["LengthIncrement"];
                            print("FileLength Year: $fileLength");
                            length = fileLength + 1;
                            print("Length Year: $length");
                            if(length == 1){
                              length = 0;
                            }
                          });
                      ...
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          ...
                          setState(() {
                            ...
                            print(indexDropDown);
                            fileLength = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["LengthIncrement"];
                            print("FileLength Month: $fileLength");
                            length = fileLength + 1;
                            print("Length Month : $length");
                            if(length == 1){
                              length = 0;
                            }
                          });
  ...
  void loadDropDownYear(){
    int x = 0;
    print(fileContent[jsonStart][0][chooseYear][11][chooseMonth][2][jsonName]["LengthIncrement"]);
    setState(() {
  ...
  Widget _despChartList(BuildContext context, int index, String nameOrder) {
    ...
    if(lengthList != index || index == 0) {
      datasTxt =
      fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]
          .keys.toList()[4 + index];

      valor1Check = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][0];
      resultCheck = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][2];
    }

    valor2Check = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["Valor"];
    result2Check = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["Result"];
    ...
    if(lengthList != index || index == 0){
      data = "Data: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Data"]}";
    }else {
      data = "Total Gasto";
    }

    if (lengthList != index || index == 0){
      sal = "Sal: R\$${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][0].toStringAsPrecision(strPrc[0])} ";
    } else{
      sal = "Sal: R\$ ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["Valor"].toStringAsPrecision(strPrc[1])}";
    }

    if(lengthList != index || index == 0){
      qntSacos = "Qnt de Sacos: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][1]}";
    }else {
      qntSacos = "Qnt de Sacos: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["Qnt"]}";
    }

    if(lengthList != index || index == 0){
      result = "Total: R\$${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][2].toStringAsPrecision(strPrc[2])} ";
    }else {
      result = "Total: R\$${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["Result"].toStringAsPrecision(strPrc[3])}";
    }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            if(lengthList != index || index == 0)(
                tabelaGastos(sal,qntSacos,result)
            )else(
                tabelaResult(sal, qntSacos, result)
            )
          ],
        ),
      ),
  ...
  Widget tabelaGastos(String sal,qntSacos,result){
    return Container(
  ...
  Widget tabelaResult(String sal,qntSacos,result){
    return  Container(
      padding: EdgeInsets.only(top: 1.5 * SizeConfig.blockSizeVertical),
      height: 12 * SizeConfig.blockSizeVertical,
      width: 50 * SizeConfig.blockSizeHorizontal,
  ...
  Future deleteButton(int indexCard ) async {
    ...
    datasTxt = fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName].keys.toList()[4+indexCard];
    final jsonFile = await JsonPath().localGAnimaisFileJson;

    for (int i = 0; i <= 2; i++){
      fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][listNameJson[i]] =
          fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][listNameJson[i]] -
              fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][i];
    }

    fileContent[jsonStart][indexYearDropDown][jsonName]["Result"] = fileContent[jsonStart][indexYearDropDown][jsonName]["Result"] -
        fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName][datasTxt]["Valores"][2];

    fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName].removeWhere((key, value) => key == datasTxt);
    ...
    fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][2][jsonName]["LengthIncrement"] = fileLength;
