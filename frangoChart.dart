...
class FrangoChart extends StatefulWidget {
  @override
  _FrangoChartState createState() => _FrangoChartState();
}

class _FrangoChartState extends State<FrangoChart> {
  ...
  Future loadJson() async {
    final jsonFile = await JsonPath().localOvoFrangoFileJson;
  ...
  Widget _despChartList(BuildContext context, int index, String nameOrder) {
    ...
    lengthList = length - 1;
    print(fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][1][jsonName]);
    ...
    if (lengthList != index || index == 0){
      frango = "Frango: R\$${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][1][jsonName][datasTxt]["Valores"][0].toStringAsPrecision(strPrc[0])} ";
    } else{
      frango = "Frango: R\$ ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][1][jsonName]["Valor"].toStringAsPrecision(strPrc[1])}";
    }

    if(lengthList != index || index == 0){
      qnt = "Quantidades: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][1][jsonName][datasTxt]["Valores"][1]}";
    }else {
      qnt = "Quantidades: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][1][jsonName]["Qnt"]}";
    }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            if(lengthList != index || index == 0)(
                tabelaGastos(frango,qnt,result)
            )else(
                tabelaResult(frango, qnt, result)
            )
          ],
        ),
      ),
