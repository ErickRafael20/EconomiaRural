...
class GasolinaChart extends StatefulWidget {
  @override
  _GasolinaChartState createState() => _GasolinaChartState();
}

class _GasolinaChartState extends State<GasolinaChart> {
  ...
  Future loadJson() async {
    final jsonFile = await JsonPath().localFileGasolinaJson;
  ...
  Widget _despChartList(BuildContext context, int index, String nameOrder) {
    ...
    if (lengthList != index || index == 0){
      nomeGasolina = "Posto: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][datasTxt]["Valores"][0]} ";
    }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            if(lengthList != index || index == 0)(
                tabelaGastos(nomeGasolina, result)
            )else(
                tabelaResult(result)
            )
          ],
  ...
  Widget tabelaGastos(String nomeGasolina, result){
    return Container(
