...
class RacaoChart extends StatefulWidget {
  @override
  _RacaoChartState createState() => _RacaoChartState();
}

class _RacaoChartState extends State<RacaoChart> {
  ...
  Widget _despChartList(BuildContext context, int index, String nameOrder) {
    ...
    if (lengthList != index || index == 0){
      racao = "Ração: R\$${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][3][jsonName][datasTxt]["Valores"][0].toStringAsPrecision(strPrc[0])} ";
    } else{
      racao = "Ração: R\$ ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][3][jsonName]["Valor"].toStringAsPrecision(strPrc[1])}";
    }

    if(lengthList != index || index == 0){
      qntSacos = "Qnt de Sacos: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][3][jsonName][datasTxt]["Valores"][1]}";
    }else {
      qntSacos = "Qnt de Sacos: ${fileContent[jsonStart][indexYearDropDown][chooseYear][indexDropDown][chooseMonth][3][jsonName]["Qnt"]}";
    }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            if(lengthList != index || index == 0)(
                tabelaGastos(racao,qntSacos,result)
            )else(
                tabelaResult(racao, qntSacos, result)
            )
          ],
        ),
      ),
  ...
  Widget tabelaGastos(String racao,qntSacos,result){
    return Container(
  ...
  Widget tabelaResult(String racao,qntSacos,result){
    return  Container(
      padding: EdgeInsets.only(top: 1.5 * SizeConfig.blockSizeVertical),
      height: 12 * SizeConfig.blockSizeVertical,
      width: 50 * SizeConfig.blockSizeHorizontal,
